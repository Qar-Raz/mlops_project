# Project Contributions

This document tracks the individual contributions of each team member to the Flora Care MLOps project.

> **Note:** This file is managed exclusively by @Qar-Raz.

## Team Members & Contributions

### 1. Qar-Raz
- Frontend
- Backend api logic
- Improved and Helped in Model serving logic
- Containerization logic and Deployment Workflow
- All testing workflows, CI workflows
- Rag Evaluation + MLflow
- Frontend Vercel Deployment
- Containerization Architecture Design


### 2. hamza7523
- Azure Configuration ACR
- AWS Configuration S3
- Ec2 Configuration
- Documentation for M1
- README (M1)
- Financially contributed for Cloud Services
- Guardrails Implementation
- 

### 3. ZainSharjeel
- Evidently AI
- Prometheus and Grafana (M2)
- M1 Documentation
- M2 Documentation
- Making Dashboards

### 4. AbdullahKhalid-projects
* CV MODEL Training
* RAG model Training
* CV model dataset research
* Pushed Models to S3
* Model Serving Logic
* Helped with backend setup
* Pipeline creation & inference testing 
* Mlflow monitoring  and its documentation
* Architecture Diagrams - dataflow, system architectural diagram
* M2 Bonus - Custom Retrievers, langchain


### 5. AnushaRandhawa
- Prometheus and Grafana for M1
- Curated Milestone 2 content
- Error debugging in docker-compose.yml, dockerfile, dependency conflicts
- SECURITY.md and guardrails documented 
- Put together README documenting Milestone 1
